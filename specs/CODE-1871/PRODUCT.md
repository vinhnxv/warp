...
- Reuse the GUI's prompt-history data source rather than introducing a second source of truth.
## Non-goals
...
- Persisting prompt history differently than it already is; this feature only reads existing history.
## Behavior
### Opening
...
10. On open, the selection defaults to the item nearest the input (the last/most-recent row). Pressing Up moves the selection toward older prompts; pressing Down moves it toward newer prompts.
...
20. Pressing Down past the last (newest) item closes the menu and restores the original buffer, matching the GUI (the newest item is at the bottom, and moving "down and out" returns to the live input). Selecting a conversation/command is not applicable here since only prompts are listed.

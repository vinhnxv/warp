...
15. Once the run ID is available, the placeholder displays `View cloud run here:` followed by the run's Oz URL.
...
19. Pressing Enter while the read-only cloud child is focused opens its current primary link. For a launched child this is the Oz run URL; for an authentication-blocked child this is the authentication URL.

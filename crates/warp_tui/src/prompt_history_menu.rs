...
//!
//! The prompt list comes from the shared [`tui_prompt_history`] helper so the TUI
//! and GUI read the same history with identical ordering, dedupe, and prefix
//! filtering. The model owns menu lifecycle, selection, preview, and buffer
//! snapshot/restore only; the terminal session view submits an accepted prompt.
use warp::editor::{CodeEditorModel, CodeEditorModelEvent};
use warp::tui_export::{tui_prompt_history, BlocklistAIHistoryEvent, BlocklistAIHistoryModel};
...
impl TuiPromptHistoryMenuModel {
    ...
    fn refresh_rows(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        let rows: Vec<TuiPromptHistoryRow> =
            tui_prompt_history(self.terminal_surface_id, &query, ctx)
                .into_iter()
...
fn reconciled_selection_index(
    ...
    if let Some(text) = previous_text {
        if let Some(index) = rows.iter().position(|row| row.text == text) {
            return Some(index);
        }
    }

...
use crate::attachment_bar::{
    TuiAttachmentBar, TuiAttachmentBarEvent, TuiAttachmentModel, TuiAttachmentPasteDisposition,
...
impl TuiView for TuiTerminalSessionView {
    ...
    fn keymap_context(&self, ctx: &AppContext) -> keymap::Context {
        ...
        if !self.process_owns_input() && !self.suggestions_mode.as_ref(ctx).mode().is_visible() {

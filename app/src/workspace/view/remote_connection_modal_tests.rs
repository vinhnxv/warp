...
fn submit_requires_server_user_and_path() {
    ...
    assert!(validate(&complete, None).can_submit());
    ...
    for incomplete in [
        ...
        assert!(
            !validate(&incomplete, None).can_submit(),
...
fn blank_port_defaults_to_22_and_non_numeric_is_rejected() {
    ...
    let padded = validate(&form("h", " 2222 ", "u", "", "/srv"), None);
    assert_eq!(padded.port_number, 2222);
    assert!(padded.can_submit());

    for bad in ["abc", "0", "70000", "-1"] {
        let result = validate(&form("h", bad, "u", "", "/srv"), None);
        assert!(
...
fn identity_path_expands_home_and_must_exist() {
    ...
    let tilde = validate(
        &form("h", "22", "u", "~/.ssh/id_ed25519", "/srv"),
        Some(home.path()),
    );
    assert_eq!(tilde.identity_error, None);
    assert!(tilde.can_submit());

    let absolute = validate(
        &form("h", "22", "u", &key.to_string_lossy(), "/srv"),
        Some(home.path()),
    );
    assert_eq!(absolute.identity_error, None);

    let missing = validate(
        &form("h", "22", "u", "~/.ssh/nope", "/srv"),
        Some(home.path()),
    );
    ...
    let empty = validate(&form("h", "22", "u", "", "/srv"), Some(home.path()));
...
fn leading_dash_in_server_or_user_is_rejected() {
    ...
    let bad_user = validate(&form("h", "22", "-oProxyCommand=x", "", "/srv"), None);

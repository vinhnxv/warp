...
pub(crate) fn initialize_app(app: &mut App) {
    ...
    // A CLI agent status change resolves the terminal's task through this
    // model, so anything that emits `CLIAgentSessionsModelEvent` needs it
    // registered. `lib.rs` registers it in the real app; the other test
    // fixtures (`test_util::terminal`, `pane_group::mod_tests`,
    // `tui_test_support`) already do the same.
    app.add_singleton_model(
        crate::ai::blocklist::local_agent_task_sync_model::LocalAgentTaskSyncModel::new,
    );
...
#[cfg(feature = "local_fs")]
#[test]
fn test_dropdown_pick_does_not_persist_default_folder_editor() {
    // R6/KTD3 regression: picking an IDE from the open-folder dropdown is a
    // one-off launch and must never write `default_folder_editor` (the review
    // caught an implementation that persisted it as "last-used").
    ...
    App::test((), |mut app| async move {
        ...
        workspace.update(&mut app, |workspace, ctx| {
            // The mock workspace has no live shell session, so no local cwd
            // resolves. Guard that assumption: a resolving target would make
            // this test launch a real editor on the test machine.
            assert!(
                workspace.resolve_open_folder_target(ctx).is_none(),
                "mock workspace must not resolve an openable folder in tests"
            );

            workspace.handle_action(&WorkspaceAction::OpenCurrentFolderIn(Editor::VSCode), ctx);

            assert!(
                !EditorSettings::as_ref(ctx)
                    .default_folder_editor
                    .is_value_explicitly_set(),
                "a dropdown pick must not write the default-folder-IDE setting"
            );
        });
    });
}

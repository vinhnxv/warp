...
fn supports_orchestration_runners_matches_flags() {
    ...
    // CloudAgentRunners flag is on, mirroring the
    // supports_orchestration_v2(orchestration_enabled) gating style. The
    // override guard must drop before a second override is set for the same
    // flag, so each case is in its own block.
    {

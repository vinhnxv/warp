...
- R12. Selecting a remote entry opens a tab that connects to the host and ends up in the entry's remote path.
...
- R18. New logic stays additive and confined to repo mode's declared glue inventory plus the add-menu change, so rebasing the patch branch onto upstream master stays routine.
...
- `DetectedEndOfSshLogin(SshLoginStatus::ReadyToWarpify)` fires once per remote login on the pane's `ModelEventDispatcher` and is the correct moment to run `cd` in the warpified remote shell (KTD7). OQ1 confirms this end-to-end during smoke.
- The `bash -s`-over-stdin probe (KTD6) requires a remote host with `bash` available on `PATH`. If a target only has a POSIX `sh`, the probe script must stay POSIX-portable; treated as an implementation detail within U5, not a scope change.
...
- **Test scenarios:**
  ...
  - Reason mapping (KTD6/A3): a stubbed timeout maps to unreachable/timeout; a stubbed `BatchMode` non-zero auth/host-key exit maps to needs-first-hand-connect; a path-not-found result maps to path-not-found — the modal shows the specific reason, not a generic "failed".
...
- SG1 resolved: the two candidate telemetry events are dropped, so `app/src/server/telemetry/events.rs` is untouched in the diff.

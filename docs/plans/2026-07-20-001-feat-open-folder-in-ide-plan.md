...
- Per-IDE brand icons for the primary button, if not already present in the icon set (start with a generic editor icon + IDE-naming tooltip).
...
- "Open with…" and per-IDE brand icons recorded under Deferred, not implemented.
- Feature flag defaults off; no user-visible change until promoted.

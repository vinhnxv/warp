...
- R5. With repo mode enabled, a Repositories section appears inside the vertical tabs panel as a fixed block between the control bar and the tab scroller, visually separated from the terminal tab list; the rest of Warp's core UI is unchanged, and with the flag off the section does not exist. *Amended 2026-07-28: only the section header shipped as a fixed block. The scroller **body** is now `render_repo_tree` — an accordion where each entry owns the nested list of its own tabs — rather than the section sitting above an unchanged tab list. The KTD2 stop condition ("if the vertical-tabs hook exceeds one small self-contained block, stop and amend R5/KTD2") did fire during U4 and the amendment was not written at the time; recorded here. The accordion was kept because a flat tab list below a repo list made the entry↔tab relationship invisible, which is the whole point of the section. Cost accepted: `vertical_tabs.rs` is +372/−194 with `repo_filter`/`flatten_repo_groups`/`in_repo_accordion` threaded through eight render functions and five items promoted to `pub(super)`, so an upstream rebase touching that file is no longer a minutes-long resolve (see the amended R9 inventory).*
...
- R10. An "All" state heads the Repositories section and is the default when no prior selection is persisted (`selected_repo_root` is NULL), including first launch: it shows the stock tab set (grouped and ungrouped tabs alike). When a selection was saved, restore it per R7. Ungrouped tabs — plain new tabs, tabs dragged in from another window, tabs orphaned by entry removal — appear only under "All". *Amended 2026-07-28: no "All" row ships in the section. "All" survives as a **state** (`selected_repo_root` NULL, still the default and still what restore falls back to) but not as a row: the accordion (amended R5) makes every entry's tabs visible at once, so a row whose only job was "show everything" read as redundant. Deselect is therefore clicking the already-selected row, or the R13 palette's "Show All Repositories" — the picker menu does still list an "All" item. Second change: loose tabs render in a permanent "Other tabs" block under **every** selection, not only under "All"; hiding the block while a repo was selected stranded the tabs with no way back to them once the horizontal strip was filtered. Consequence to accept: there is no highlighted affordance telling a first-launch user they are in the "All" state — see amended AE5.*
...
- R9. New logic lives in a dedicated workspace crate and new additive files under `app/`; upstream files receive only small, self-contained glue edits at the declared inventory below, so rebasing the patch branch onto fresh upstream master routinely succeeds with conflicts confined to those glue points.
  ...
  *Amended 2026-07-28 — inventory reconciled against the shipped diff.*

  Two entries understate what landed:
  - `vertical_tabs.rs` is not "one flag-gated hook" — see amended R5/KTD2. This is the one file where the R9 promise (rebase conflicts resolve in minutes) no longer holds.
  - `app/src/workspace/view.rs` grew ~562 lines rather than thin call sites, mostly the horizontal-strip filter and restore paths.

  Files touched that this table never listed:

  | File | Touch | Origin |
  |---|---|---|
  | `app/src/lib.rs` | One line wiring the remote-tab path | remote-SSH plan U8 (disclosed in its own audit) |
  | `app/src/menu.rs` | `image_icon()` / `right_side_icon()` accessors | open-folder plan (brand icons) |
  | `app/src/view_components/action_button.rs`, `compactible_action_button.rs`, `compactible_split_action_button.rs` | Image-icon plumbing + tooltip positioning provider | open-folder plan (brand icons; tooltip flicker fix) |
  | `app/src/server/telemetry/events.rs` | `OpenedFolderInIde` event | open-folder plan R8 |
  | `crates/warp_assets/build.rs` | **New file** — `rerun-if-changed` so bundled assets rebuild | open-folder plan (brand icons) |
  | `app/src/util/openable_file_type_tests.rs` | Mechanical ripple | open-folder plan |

  All six come from the sibling plans rather than this one, but they land on the same patch branch and therefore in the same rebase surface, so a future rebase must expect them. `crates/warp_assets/build.rs` is the only genuine upstream-infrastructure change in the set and is the one worth upstreaming separately.
...
- AE5. **Covers R10.** Given the flag is on and no persisted selection (`selected_repo_root` is NULL), when Warp launches, then the tab UI shows the stock tab set and no entry row is highlighted. *Amended 2026-07-28: was "and 'All' is highlighted"; unimplementable once the "All" row was dropped (amended R10). The observable half — stock tab set on a NULL selection — is unchanged and is what `test_no_selection_shows_stock_tab_set` asserts.*
- AE6. **Covers R12.** Given a tab opened under repo A, when the user runs `cd` to a directory outside A, then the tab remains in A's group. *Amended 2026-07-28 — **known divergence, not yet accepted.** Membership is still static as R12/KTD6 require (`group_id` is never rewritten), but the **display** partition is live-cwd: `repo_mode_bound_tab_owner` attributes a bound tab to the deepest registered ancestor of its current cwd, so with both `/repo` and `/repo/sub` registered, a tab bound to `/repo` that cd's into `/repo/sub` renders under `/repo/sub`. `test_bound_tab_owner_rules` currently pins that behavior, i.e. asserts the opposite of this AE. Two defects trace to it: `tab_bar_slots` merges the resulting non-adjacent group members into one slot and renders a hidden tab while dropping a visible one, and `select_repo_mode_entry` opens a redundant terminal when an entry's tabs have all been attributed elsewhere. Resolving this is a design decision — keep live-cwd display and fix both consumers, or revert to group-binding attribution and let R12/AE6 stand as written — deferred to the remediation plan; do not treat this note as ratifying the current behavior.*
...
- ~~Cursor-style nested tab/session list under each entry inside the section (v2; the data model here already supports it).~~ *Amended 2026-07-28: pulled into v1 and shipped — it is the accordion in amended R5. Deferring it left a repo list sitting above a flat tab list with no visible relationship between them, which defeated the section. Two further unplanned additions rode along and are recorded here rather than back-dated into the requirements: per-entry git diff-stat and clickable PR badges on each row (`repo_mode_entry_badges` + the `VerticalTabsChipEntrypoint::RepoSidebar` telemetry variant), and the "Other tabs" block's "+ New" loose-terminal button.*
...
- **KTD6 — Static membership; no `WorkingDirectories` integration.** Membership is assigned at open time from the selected entry. Tracking cwd drift would pull in the per-pane cwd module for marginal value; drag-in tabs from other windows arrive ungrouped and land under "All" (upstream drag never transfers `group_id`). *Amended 2026-07-28 — **partially contradicted, resolution pending.** Storage honors this (`group_id` is written once and never rewritten), but rendering does not: `repo_mode_tab_partition` reads `canonical_session_pwd_if_local` on every frame and re-attributes bound tabs by live cwd. That is the cwd tracking this KTD declined, arrived at via the session pwd rather than `WorkingDirectories`. See amended AE6 for the two defects it causes and the pending keep-or-revert decision.*
- **KTD7 — New crate `crates/repo_mode` holds entry identity and path rules only.** Canonicalization/dedup (via `dunce::canonicalize`, used consistently everywhere including `ProjectManagementModel` upsert/remove), dead-path check, git-or-folder kind via a `.git`-exists check (file or directory, covering linked worktrees; *amended 2026-07-20:* `repo_metadata` not needed for classification) — pure, unit-testable, no `app/` dependency. **All/Repo selection state machine and filtering live entirely in `RepoModeModel` / additive `app/` modules** — not in the crate. *Amended 2026-07-28: no `RepoModeModel` type was created. The state machine ships as an `impl Workspace` block in the additive module `app/src/workspace/view/repo_mode_model.rs`, and selection is the `Workspace.selected_repo_root` field rather than a per-window model. The "not in the crate" half — which is what protects the crate's purity and unit-testability — holds exactly as written; only the container differs.* The workspace member glob (`crates/*`) auto-includes it; glue is one `[workspace.dependencies]` line plus one `app/Cargo.toml` line. Size gate: if domain logic stays under ~400 LOC with a single consumer, folding into `app/src/workspace/repo_mode/` is acceptable during U1 without amending R9.
...
- **Approach:** Mirror conversation-list section header styling (KTD2). Row schema: primary = directory basename; secondary = optional shortened path (tooltip on overflow); icon = repo vs folder; git branch only for repos (omit for folders per R2); reuse vertical-tabs hover tokens (`fg_overlay_*`). States: default, hover, selected, selected+hover, dead-path (dimmed + inline remove). Healthy-entry remove via context menu. Keyboard v1 (*amended 2026-07-20*): keyboard-only switching is provided by the "Select Repository..." picker menu (native Menu arrow/Enter/Esc handling) rather than roving focus inside the section — the section itself is pointer-driven; roving focus would require promoting it to its own focusable view and is deferred. Style with `appearance.theme()` accessors only.
...
|---|---|---|
| Unit tests | `cargo nextest run -p repo_mode -p persistence` and app workspace / view tests (*amended 2026-07-20:* repo_sidebar render tests deferred) | U1, U2, U3, U4, U5 |
| Integration | repo-mode restoration test in `crates/integration` suite (*amended 2026-07-20:* deferred — restore covered by view/sqlite unit tests; revisit if restore regresses) | U6 |

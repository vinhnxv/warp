...
- AE6. **Covers R12.** Given a tab opened under repo A, when the user runs `cd` to a directory outside A, then the tab remains in A's group. *Amended 2026-07-28, resolved 2026-07-30 as originally written.* The display partition briefly diverged from this: `repo_mode_bound_tab_owner` attributed a bound tab to the deepest registered ancestor of its live cwd, so a tab bound to `/repo` that cd'd into a registered `/repo/sub` rendered under `/repo/sub`, and `test_bound_tab_owner_rules` pinned that behavior. Two defects traced to it — `tab_bar_slots` merged the resulting non-adjacent members into one slot, rendering a hidden tab while dropping a visible one, and `select_repo_mode_entry` opened a redundant terminal for an entry whose tabs had all been attributed elsewhere. The remediation plan resolved it by reverting to group-binding attribution (U3), so this AE now holds for display as well as storage; the slot model was fixed independently (U1).
...
- **KTD6 — Static membership; no `WorkingDirectories` integration.** Membership is assigned at open time from the selected entry. Tracking cwd drift would pull in the per-pane cwd module for marginal value; drag-in tabs from other windows arrive ungrouped and land under "All" (upstream drag never transfers `group_id`). *Amended 2026-07-28, resolved 2026-07-30 as originally written.* Storage always honored this, but rendering did not: `repo_mode_tab_partition` read `canonical_session_pwd_if_local` on every frame and re-attributed bound tabs by live cwd — the cwd tracking this KTD declined, arrived at via the session pwd rather than `WorkingDirectories`. The remediation plan removed that read (U3); the partition now touches no terminal state at all.
